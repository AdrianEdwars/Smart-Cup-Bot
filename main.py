...
for event in VkLongPoll(session).listen():
    if event.type == VkEventType.MESSAGE_NEW and event.to_me:
        ...
        if lang == 0:
            if text == 'хватит ❌' or (text == 'история' and status != 1):
...
{ANSWERS[0].capitalize()} и {ANSWERS[2]} отправились в путешествие через темный лес, который казался бесконечным. Вскоре они пришли к {ANSWERS[6]}, который был полностью построен из {ANSWERS[4]}. {ANSWERS[2]} сказал: "Мы пришли за артефактом под названием "{ANSWERS[5]}", который был спрятан внутри этого здания много лет назад". {ANSWERS[0]} почувствовал себя немного напуганным, но его любопытство взяло верх, и он решил идти внутрь здания вместе с существом.
Они преодолели множество опасностей, прежде чем нашли {ANSWERS[5][:-1]}у. Но внезапно {ANSWERS[0]} услышал странный звук, и {ANSWERS[2]} исчез. {ANSWERS[0]} остался один в здании, но он знал, что должен вернуться домой и рассказать всем свою историю.""")
            ...
            if text == 'stop ❌' or (text == 'history' and status != 1):
                keyboard = VkKeyboard(one_time=True)
            ...
            if text == 'change language 🇷🇺' and status == 0:
                lang = 0
                send_message(user_id, "Вы успешно изменили язык на русский (You succesfully changed language to Russian) Напишите 'история' чтобы продолжить")
